...
def strf_date(x):
    if pd.isnull(x) or (type(x) == str and len(x) == 0):
        return None
...
def data_ingestion(db: Session, path: str = 'utils/IDX_OUTPUT_NEW_REPORT.xlsx'):
    ...
    for year in year_dict.keys():
        ...
        current_df["order_number"] = current_df["order_number"].apply(lambda x: "NYUSH" + str(year) + x)
    ...
    for idx, row in check_prev.iterrows():
        if (check_curr.iloc[idx - deleted_rows]["BSN"] == row["bsn"] and
                check_curr.iloc[idx - deleted_rows]["Z68_ORDER_NUMBER"] == row["order_number"]):
